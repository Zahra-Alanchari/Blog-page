...
function showData() {
  ...
  console.log(JSON.parse(localCommentsData), "local");
  JSON.parse(localCommentsData).comments.map((comments) => {
  ...
  replyToComment.forEach((item) => {
    item.addEventListener("click", () => ReplyClick(item));
  });

  //   replyToPost.forEach((item) => {
  //     item.addEventListener("click", ReplyClick2);

  //     function ReplyClick2() {
...
function ReplyClick(item) {
  ...
  if (!post.querySelector(".addDiv")) {
    ...
    saveReplyBtn.addEventListener("click", () => {
      ...
      storedData.comments.forEach((commentItem) => {
        // console.log(commentItem, "id")
        // console.log(item, "id2")
        if ((commentItem.user.username = "amyrobson")) {
          commentItem.content = contentOfTextarea;
        }
      });
      // storedData.comments[0].content = contentOfTextarea
